...
if("null" != $PAGE && $PAGE)
{
    $mysql  = "SELECT t.spanishTitle AS titulo, te.spanishText AS descripcion, md.robotIndex, md.robotFollow, md.catId, md.prodId "
                ."FROM metadata md "
                ."INNER JOIN titles t ON t.id = md.title "
                ."INNER JOIN texts te ON te.id = md.description "
                ."LEFT JOIN categories c ON c.id = md.catId "
                ."LEFT JOIN titles t2 ON c.name = t2.id "
                ."LEFT JOIN product p ON p.id = md.prodId "
                ."WHERE t2.spanishTitle = '$PAGE' OR p.slug = '$PAGE'" ;
